...
**Phase 2: Bot Decision Engine — COMPLETE**
- `enumerateLegalActions()`: Enumerates all valid race actions (bet at 3 wager levels, share, drug, shop share, market buy, skip)
- `evaluateAction()`: Scores each action using personality weight vectors, win probabilities, game phase, coin position
- `pickWeightedTop()`: Selects from top-3 scored actions with weighted randomness
- `executeBotAction()`: Executes chosen action via same game functions as humans
- `executeBotTurn()`: Orchestrates think delay → evaluate → execute → roll
- `enumerateDowntimeActions()` / `evaluateDowntimeAction()` / `executeBotDowntime()`: Full downtime decision engine
- Bot market behavior: Sells based on internal valuation, prices based on personality

**Phase 3: Turn Flow Integration — COMPLETE**
- `checkAndRunBotTurn()` auto-triggers after `nextTurn()`, game start, and race start
- `showDowntimeScreen()` auto-submits bot actions with staggered 400ms delays
- `renderBettingPanel()` returns early during bot turns (overlay handles display)
- Human action functions block during bot turns via `isCurrentPlayerBot()` guard
- Bot thinking indicator shows in betting panel, then switches to action toast

**Phase 4: Polish & Feel — COMPLETE**
- Bot archetype badges (icon + color) in: activity feed, downtime panels, game over podium
- CSS thinking animation (pulsing dots), action toast styling
- High score works for solo human vs bots (counts human players only)
- Archetype descriptions present from Phase 1 setup screen
